...
uv run lokay-tick-all --live   # only after dry-run looks good
```

Default is **dry-run** unless `--live` is passed. Schedule only
`lokay-tick-all`; run the individual ticks above manually when diagnosing
...
- The installed `lokay-tick-all` launchd job runs naturally for at least two consecutive `StartInterval` windows; manual `kickstart` does not count.
