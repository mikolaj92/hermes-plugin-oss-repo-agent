"""Shared CLI helpers for lokay tick entrypoints."""
...
def add_common_flags(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
    p.add_argument(
        "--config",
        default=None,
        help="Path to config.toml (default: ~/.hermes/lokay/config.toml)",
    )
    p.add_argument(
        "--db",
        default=None,
        help="Fala SQLite path (default: ~/.hermes/lokay/fala/state.sqlite)",
    )
    p.add_argument(
        "--dry-run",
        action="store_true",
