...
async def run_cleanup_flow(
    *,
    ...
    result = await run_repo_agent_path(
        ...
        effector_inputs={
            "parse_issue_from_branch": {"branch": resolved_branch, "dry_run": is_dry, "repo": resolved_repo, "board": (cfg.repos[0].board if cfg.repos else "")},
            "check_issue_closed": {"repo": resolved_repo, "dry_run": is_dry, "board": (cfg.repos[0].board if cfg.repos else "")},
            "check_no_open_pr": {"repo": resolved_repo, "branch": resolved_branch, "dry_run": is_dry, "board": (cfg.repos[0].board if cfg.repos else "")},
            "remove_worktree": {"clone_path": resolved_clone, "worktree_path": wt, "dry_run": is_dry, "require_safe": True, "repo": resolved_repo, "board": (cfg.repos[0].board if cfg.repos else "")},
            "delete_local_fix_branch": {"clone_path": resolved_clone, "branch": resolved_branch, "dry_run": is_dry, "repo": resolved_repo, "board": (cfg.repos[0].board if cfg.repos else "")},
            "release_active_issue_claim": {"claim_path": claim, "repo": resolved_repo, "dry_run": is_dry, "board": (cfg.repos[0].board if cfg.repos else "")},
        },
