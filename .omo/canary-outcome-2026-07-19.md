...
- A direct deployed-worker invocation correctly failed because `uv` attempted to create `.venv` inside the immutable candidate. Re-running with the exact launchd `UV_PROJECT_ENVIRONMENT` and `UV_CACHE_DIR` completed live run `auto-worker-20260723T131038Z-45a00ae6` with no failed or waiting steps; this was an environment-path error, not a CPython 3.14.3 compatibility failure.
