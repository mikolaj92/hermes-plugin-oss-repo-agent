...
- `deployment/current` resolves to that immutable version. The active smoke script was installed from the same committed source after the pre-install smoke failed on that sole hash mismatch; full five-script/config/template parity then reported `ok=true` with no errors.
