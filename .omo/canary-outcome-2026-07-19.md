...
- Fresh verification retained fail-closed behavior: the three focused launch-domain regressions passed against the tested source/code tree at `aadebff77a7a2022d19afbcb183aaa0feb560eb3`; the initial evidence-only documentation commit was then published separately as `de518e04a9f2615a55e8f1de7ce7c599f6150583`. Active parity still reports drift for health/status, and source health/status both exit `1` on GUI-domain ambiguity. The configured disk gate also remains below `5 GiB` with approximately `3.4 GiB` available.
...
## Completed immutable promotion 2026-07-25

**Final disposition: COMPLETE — PROMOTED.**

- Deployed code provenance is clean and published: candidate `f35056597dd7a87c1d630557d44b740d1161087dd466a6ea974c7914217c9110` binds deployed source revision `2f39271bde70e9c71e244a664912ee868d0a8407`, which was the clean `main` and `origin/main` tip at promotion. The later documentation-only commit `cedf8abaf3dc39775f9f0b492d3f71ba88561720` moved `main` and `origin/main` without changing deployed code; exact repository-HEAD equality is therefore not claimed.
- Immutable candidate `f35056597dd7a87c1d630557d44b740d1161087dd466a6ea974c7914217c9110` passed validation before and after promotion. Its stable identity binds plugin commit `2f39271bde70e9c71e244a664912ee868d0a8407`, Fala `0.7.9` at `69bc2ec9d4cdf61773114847c0c582fb2652296d`, the configured live database, and policy `automerge=false`, `executor_enabled=false`, `require_human_approval=true`, `require_checks=true`, `require_test_evidence=true`.
- `deployment/current` resolves to immutable version `f35056597dd7a87c1d630557d44b740d1161087dd466a6ea974c7914217c9110`. The installed canonical Fala plist matches the promoted version runtime identity at SHA-256 `e5a80f924b1ebf39c64e6ddc1f2a5fe2b8a48b19bf39594d486305e085d13ba4`. `previous.json` preserves candidate `53f59f83f016841a3c2d88a42c5b189c0b07bf1d6d49d7e36696351f58d110c3` and its exact prior `user/501` launchd state for rollback.
- Launchd reconciliation is authoritative for this host context: `user/501` is observable with exactly one loaded canonical Fala job; `gui/501` returns the recognized unavailable-domain result and is not counted as an ambiguous second domain. Legacy mutators and repair health are not loaded.
- Two natural scheduled live runs completed after cutover with no unresolved, failed, or waiting processes: `auto-worker-20260725T060324Z-b8b61318` and `auto-worker-20260725T061341Z-c033ee49`. Both were successful no-op ticks; scheduler health is proven without claiming issue resolution.
- Issue `#8` is closed and PR `#9` is merged at `49d21868ebd0f1ea3913efdcba6582a14e39f900`. The durable terminal cleanup receipt reports `NO_TARGET_RECONCILED`, zero open matching PRs, absent claim/worktree targets, and the explicitly authorized retained remote branch.
- Final acceptance at `2026-07-25T06:20Z` passed candidate validation for both candidate and deployed version, deployment parity, health with `failures=0`, status with `Gate summary failures=0`, and bounded smoke. The focused final regression set passed `160` tests. Two operational warnings remain non-blocking: a Hermes update is available and the unrelated Hermes-update log is stale.
- Independent final audits approved launchd handling/code quality and observed scope fidelity. An earlier runtime audit rejected because it compared the pre-promotion candidate manifest with the runtime-rebound version manifest; the authoritative deployed-version validator, health gate, status gate, installed-plist hash, and `deployment/current` all agree on `f35056597dd7a87c1d630557d44b740d1161087dd466a6ea974c7914217c9110`. Absolute historical non-mutation of every unrelated user artifact remains unprovable without a full before/after content inventory; contemporaneous observed changes were confined to managed repo-agent deployment, runtime, log, rollback, and canonical plist paths.
